...
-- 模块19：装备错误提醒（独立子分类页面）
L["EA_Name"] = "装备错误提醒"
L["EA_Desc"] = "检测到明显的装备错误时在屏幕上显示白色文字提醒（战斗中自动隐藏，脱战恢复），多条提醒以设定位置为顶部向下堆叠。检测内容：1) 已学习工程学但未装备腰带，或腰带未附魔「氮气推进器」；2) 未装备主手武器，或双持专精副手未装备武器；3) 武器主属性与当前专精不符；4) 刺杀/敏锐贼未双持匕首；5) 奶骑/防护骑/防护战未使用单手武器+盾牌；6) 生存猎未装备双手近战武器。"
L["EA_PosX"] = "水平位置 (X)"
L["EA_PosY"] = "垂直位置 (Y)"
