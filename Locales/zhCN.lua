...
L["CharacterStats_Desc"] = "在屏幕上常态显示人物属性面板（无边框背景，左上角锚定向右下扩展），依次为：主属性、副属性（暴击/急速/精通/全能）、第三属性（吸血/闪避/加速，非0时显示）、坦克属性（躲闪/招架/格挡，仅坦克专精显示）、移速（实时移动速度百分比，每秒刷新）。数值与属性名称同色并左对齐展示（数值列与名称列间距固定）。下方可调整位置（屏幕中心为原点）、字号、文字描边与行间距。"
L["CharacterStats_PosX"] = "面板水平位置 (X)"
L["CharacterStats_PosY"] = "面板垂直位置 (Y)"
L["CharacterStats_FontSize"] = "字号"
L["CharacterStats_Outline"] = "文字描边"
L["CS_OutlineNone"] = "无"
L["CS_OutlineThin"] = "细描边"
L["CS_OutlineThick"] = "粗描边"
...
L["TooltipEnhance_Desc"] = "增强鼠标提示框：姓名按职业染色、隐藏头衔、同服隐藏服务器名、显示<离开><忙碌><离线>状态；公会行显示会阶（公会名~会阶名）；等级行隐藏\"等级\"\"玩家\"并在职业前合并专精；阵营改为右上角徽记；显示大秘境分数/史诗钥匙/物品等级(套装数/5)、各地下城限时成绩、当前赛季团本进度、目标的目标。可调整提示框字号。"
L["TE_FontSize"] = "提示框字号"
